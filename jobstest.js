...
(async () => {
  ...
  const post=async(title,desc,budget)=>{ await p.goto(url+'#post-job'); await p.waitForTimeout(300); await p.fill('#jobTitle',title); await p.fill('#jobDesc',desc); await p.fill('#jobBudget',budget||'€300'); await p.click('#publishJob'); await p.waitForTimeout(900); };
